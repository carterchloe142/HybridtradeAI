...
export default function HeroSection() {
  return (
    <section className="relative w-full min-h-[90vh] bg-[#050A18] overflow-hidden flex flex-col md:flex-row items-center pt-24 px-6 md:px-16 lg:px-24">
      ...
      <div className="relative z-10 w-full md:w-1/2 flex flex-col gap-8 md:pr-10">
        ...
        >
          ...
          <Link href="#platform" className="px-8 py-4 border border-white/10 text-white font-semibold rounded-xl hover:bg-white/5 transition-colors flex items-center gap-2">
        ...
        <motion.div 
            ...
            className="pt-8 border-t border-white/5 flex items-center gap-6"
        >
            <div className="text-sm text-slate-500">Trusted by 10,000+ traders from:</div>
            <div className="flex -space-x-4">
                 {[1,2,3,4].map((i) => (
                     <div key={i} className="w-10 h-10 rounded-full bg-slate-800 border-2 border-[#050A18] flex items-center justify-center text-xs text-slate-500 font-bold">
                        U{i}
                     </div>
                 ))}
                 <div className="w-10 h-10 rounded-full bg-slate-800 border-2 border-[#050A18] flex items-center justify-center text-xs text-white font-bold">
                    +10k
                 </div>
            </div>
        </motion.div>
      ...
      <div className="relative z-10 w-full md:w-1/2 h-[500px] md:h-[700px] flex items-center justify-center">
        ...
        <motion.div 
            ...
            className="absolute top-20 right-10 bg-black/40 backdrop-blur-md border border-white/10 p-4 rounded-2xl shadow-xl z-20"
        >
            <div className="flex items-center gap-3">
                <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
                <span className="text-sm text-slate-300">AI Bot Active</span>
            </div>
            <div className="text-2xl font-bold text-white mt-1">+12.4% <span className="text-xs font-normal text-slate-400">/ 24h</span></div>
        </motion.div>

        <motion.div 
            animate={{ y: [0, 15, 0] }}
            transition={{ duration: 5, repeat: Infinity, ease: "easeInOut", delay: 1 }}
            className="absolute bottom-32 left-0 bg-black/40 backdrop-blur-md border border-white/10 p-4 rounded-2xl shadow-xl z-20"
        >
            <div className="text-xs text-slate-400 mb-1">Total Volume</div>
            <div className="text-xl font-bold text-white">$482,920.54</div>
        </motion.div>
      </div>
    </section>
  )
