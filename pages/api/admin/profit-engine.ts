...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const CONFIG_KEY = 'profit_engine_config';
  ...
  if (req.method === 'GET') {
      ...
      let config = { fee_percent: 0, reserve_percent: 0, cycle_length_days: 7 };
      if (val) {
          try {
              // The value might be just the string if stored as text, or JSON if stored as JSONB.
              // Assuming 'value' column is text.
              config = JSON.parse(val);
          } catch (e) {
              // If it's not JSON, maybe it's something else? But we always save JSON.
          }
      }
