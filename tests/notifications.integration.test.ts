...
import { GET } from '../app/api/user/notifications/stream/route'
...
describe('SSE replay smoke test', () => {
  it('streams replay when lastEventId is provided', async () => {
    ...
    const req = new NextRequest(new Request(`http://localhost/api/user/notifications/stream?lastEventId=${last}&token=test-token`))
    const res = await GET(req)
    const text = await collectFirstChunk(res.body as any)
    expect(text).toContain('data:') // We should get replay data
