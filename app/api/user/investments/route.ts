...
export async function GET(req: NextRequest) {
  ...
  // Prefer lowercase snake_case table first
  ...
  if (items && items.length > 0 && 'user_id' in (items[0] || {})) {
    items = items.map((i: any) => ({
      ...
      userId: i.user_id,
      planId: i.plan_id,
      amount: i.amount_usd || i.amount || i.principal, 
      status: String(i.status || '').toUpperCase(),
      ...
      plan: i.plan ? {
        ...
        roiMinPct: i.plan.roi_min_pct,
        roiMaxPct: i.plan.roi_max_pct
      } : null
    }))
  }
