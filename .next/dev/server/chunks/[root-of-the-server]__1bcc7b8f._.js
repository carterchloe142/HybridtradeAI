...
"[project]/pages/api/admin/credit-user.ts [api] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function handler(req, res) {
    if (req.method === 'GET') {
        ...
        if (!admin.ok) return res.status(403).json({
        ...
        try {
            const [actions, total] = await Promise.all([
                __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$prisma$2e$ts__$5b$api$5d$__$28$ecmascript$29$__["default"].adminAction.findMany({
                    where: {
                ...
                __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$prisma$2e$ts__$5b$api$5d$__$28$ecmascript$29$__["default"].adminAction.count({
                    where: {
                        action: 'MANUAL_CREDIT'
                    }
                })
        ...
        const { data: txs, error: txErr, count } = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabaseServer$2e$ts__$5b$api$5d$__$28$ecmascript$29$__["supabaseServer"].from('transactions').select('id,user_id,amount,currency,meta,created_at', {
            count: 'exact'
        }).eq('type', 'admin_credit').order('created_at', {
            ascending: false
        }).range(skip, skip + limit - 1);
        if (txErr) return res.json({
            actions: [],
            total: 0,
            page,
            limit
        });
        const actions = (txs || []).map((t)=>({
                id: String(t.id),
    ...
    const admin = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$adminAuth$2e$ts__$5b$api$5d$__$28$ecmascript$29$__["requireAdmin"])(req);
    if (!admin.ok || !admin.userId) return res.status(403).json({
        error: admin.error || 'forbidden'
    });
    const adminId = admin.userId;
    const { userId: rawUserId, email: rawEmail, amount } = req.body ?? {};
    const note = req.body?.note ?? req.body?.description ?? undefined;
    if (typeof amount !== 'number') return res.status(400).json({
    ...
    try {
        ...
        const resolvedUserId = await resolveSupabaseUserId({
            userId: rawUserId,
            email: rawEmail
        });
        if (!resolvedUserId) return res.status(400).json({
            error: 'invalid_user_identifier'
        });
        if (threshold && amount > threshold) {
            try {
                const pending = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$prisma$2e$ts__$5b$api$5d$__$28$ecmascript$29$__["default"].adminAction.create({
                    data: {
                        adminId,
                ...
                const { data: txInserted } = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabaseServer$2e$ts__$5b$api$5d$__$28$ecmascript$29$__["supabaseServer"].from('transactions').insert({
        ...
        const { data: existing, error: selectErr } = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabaseServer$2e$ts__$5b$api$5d$__$28$ecmascript$29$__["supabaseServer"].from('wallets').select('id,balance').eq('user_id', resolvedUserId).eq('currency', currency).maybeSingle();
        ...
        if (!walletId) {
            const { data: inserted, error: insertErr } = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabaseServer$2e$ts__$5b$api$5d$__$28$ecmascript$29$__["supabaseServer"].from('wallets').insert({
                user_id: resolvedUserId,
                currency,
                balance: 0
            }).select().maybeSingle();
        ...
        const { error: updateErr } = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabaseServer$2e$ts__$5b$api$5d$__$28$ecmascript$29$__["supabaseServer"].from('wallets').update({
            balance: newAmount
        }).eq('id', walletId);
        ...
        let txn = null;
        try {
            txn = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$prisma$2e$ts__$5b$api$5d$__$28$ecmascript$29$__["default"].walletTransaction.create({
                data: {
                    walletId: walletId,
                    amount: amount.toString(),
                    type: 'CREDIT',
        ...
        try {
            await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$prisma$2e$ts__$5b$api$5d$__$28$ecmascript$29$__["default"].adminAction.create({
                data: {
                    adminId,
                    userId: resolvedUserId,
                    amount: amount.toString(),
        ...
        if (msg.includes('wallet_select_failed')) return res.status(500).json({
            error: msg
        });
        if (msg.includes('wallet_create_failed')) return res.status(500).json({
            error: msg
        });
        if (msg.includes('wallet_update_failed')) return res.status(500).json({
            error: msg
        });
        if (msg.includes('transactions_insert_failed')) return res.status(500).json({
            error: msg
        });
...
async function resolveSupabaseUserId({ userId, email }) {
    // Case 1: Provided a proper Supabase user UUID
    if (isUuidLike(userId)) return userId;
    // Case 2: Provided an email directly
    const emailToUse = isEmailLike(email) ? email : isEmailLike(userId) ? userId : '';
    if (emailToUse) {
        const { data, error } = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabaseServer$2e$ts__$5b$api$5d$__$28$ecmascript$29$__["supabaseServer"].from('profiles').select('user_id').eq('email', emailToUse).maybeSingle();
        ...
        try {
            ...
            const found = users.find((u)=>String(u?.email || '').toLowerCase() === emailToUse.toLowerCase());
    ...
    if (userId && userId.startsWith('c') && userId.length >= 24) {
        try {
            ...
            if (emailFromPrisma) {
                const { data, error } = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabaseServer$2e$ts__$5b$api$5d$__$28$ecmascript$29$__["supabaseServer"].from('profiles').select('user_id').eq('email', emailFromPrisma).maybeSingle();
                if (!error && data?.user_id) return data.user_id;
                try {
                    const adminRes = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabaseServer$2e$ts__$5b$api$5d$__$28$ecmascript$29$__["supabaseServer"].auth?.admin?.listUsers?.({
                        page: 1,
                    ...
                    const users = adminRes?.data?.users || adminRes?.users || [];
                    const found = users.find((u)=>String(u?.email || '').toLowerCase() === String(emailFromPrisma).toLowerCase());
                    if (found?.id) return String(found.id);
                } catch  {}
            }
        } catch  {}
...
//# sourceMappingURL=%5Broot-of-the-server%5D__1bcc7b8f._.js.map